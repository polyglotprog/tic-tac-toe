...
def tic_tac_toe(game_text, game_board, players):
  ...
  while not game_over:
    ...
    player_move = get_player_move_from_input(game_text, game_board, player)
...
def ask_if_ready_to_play(game_text):
  while True:
    ready = input(game_text['ready']).lower()
    if ready == 'y' or ready == 'n':
      return ready
    elif ready == 'quit' or ready == 'q':
      say_goodbye()
    else:
      print(game_text['y_or_n'])

def get_player_move_from_input(game_text, game_board, player):
  while True:
    try:
...
if __name__ == '__main__':
  ...
  ready = ask_if_ready_to_play(GAME_TEXT)
